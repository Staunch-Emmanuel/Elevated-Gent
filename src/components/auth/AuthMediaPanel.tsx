...
export default function AuthMediaPanel() {
  ...
  const desktopVideo = useMemo(() => (settings.videoUrl || '').trim(), [settings.videoUrl])
  const mobileVideo = useMemo(() => (settings.mobileVideoUrl || '').trim(), [settings.mobileVideoUrl])
  const poster = useMemo(() => (settings.posterImageUrl || '').trim(), [settings.posterImageUrl])
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-black">
        ...
        <div className="absolute bottom-8 left-8 right-8 text-white">
          ...
          <p className="text-white/90 font-serif">
            {settings.subheadline || 'Professional styling services for the modern gentleman'}
          </p>
        </div>
      </div>
